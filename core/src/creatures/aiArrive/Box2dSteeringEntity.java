...
//	}
	...
	public Location<Vector2> newLocation () {
		return null; // new Box2dLocation();
	}

	@Override
	public float vectorToAngle (Vector2 vector) {
		return 0; // Box2dSteeringUtils.vectorToAngle(vector);
	}

	@Override
	public Vector2 angleToVector (Vector2 outVector, float angle) {
		return null; //Box2dSteeringUtils.angleToVector(outVector, angle);
	}
	...
	public void update (float deltaTime) {
		if (steeringBehavior != null) {
			...
			applySteering(steeringOutput, deltaTime);
				...
				wrapAround(GameInfo.pixelsToMeters(Gdx.graphics.getWidth()*2),
				GameInfo.pixelsToMeters(Gdx.graphics.getHeight()));
	...
	protected void applySteering (SteeringAcceleration<Vector2> steering, float deltaTime) {
		...
		if (!steeringOutput.linear.isZero()) {
			...
			steeringOutput.linear.y = 0 ;  // vypnuta osa Y koly tomy aby nelietal objekt
	...
	public void draw (Batch batch) {
		...
		batch.draw(region, //
				GameInfo.metersToPixels(pos.x) - ox, GameInfo.metersToPixels(pos.y) - oy, //
			ox, oy, //
			w, h, //
			1, 1, //
